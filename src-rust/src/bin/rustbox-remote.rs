// rustbox-remote - send a command to a running Rustbox-rs instance.
//
// Sends the command to the root window as one or more ClientMessage events of
// type "Rustbox/remote" (format 8, up to 20 bytes per message), terminated by
...
use rustbox_rs::x11::{Atom, X11Connection};
...
fn main() -> Result<(), anyhow::Error> {
    ...
    if rest.is_empty() {
        eprintln!("Usage: rustbox-remote [-socket <path>] [-display <d>] <command>");
    ...
    let cmd_atom = conn.atoms().get(Atom::RustboxRemoteCmd);
    if cmd_atom == x11rb::NONE {
        anyhow::bail!("RUSTBOX_REMOTE_CMD atom not available");
    }
